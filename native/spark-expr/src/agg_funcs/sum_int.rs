...
use arrow::array::{cast::AsArray, types::Decimal128Type, Array, ArrayRef, BooleanArray, Decimal128Array, Int64Array};
...
impl AggregateUDFImpl for SumInteger {
    ...
    fn create_groups_accumulator(
        ...
        Ok(Box::new(SumIntegerGroupsAccumulator::new(DataType::Null)))
    }
...
impl SumIntegerAccumulator {
    ...
    fn update_single(&mut self, values: &Int64Array, idx: usize) {
        let v = unsafe { values.value_unchecked(idx) };
        let (new_sum, is_overflow) = self.sum.overflowing_add(v as i128);
...
impl Accumulator for SumIntegerAccumulator {
    fn update_batch(&mut self, values: &[ArrayRef]) -> DFResult<()> {
        ...
        let values = &values[0];
        let data = values.as_primitive::<Int64Type>();
    ...
    fn evaluate(&mut self) -> DFResult<ScalarValue> {
            ...
            || !self.is_not_null
        {
            ...
            ScalarValue::try_new_decimal128(self.sum)
        }
    }
    ...
    fn state(&mut self) -> DFResult<Vec<ScalarValue>> {
        let sum = if self.is_not_null {
            ScalarValue::try_new_null(self.sum)?
        } else {
            ScalarValue::new_primitive::<Decimal128Type>(
                None,
                &DataType::Int64,
...
struct SumIntegerGroupsAccumulator {
...
impl SumIntegerGroupsAccumulator {
...
mod tests {
    ...
    fn invalid_data_type() {
        assert!(SumInteger::try_new(DataType::Int32).is_err());
    }
